...
| Real Estate & Tax Records | `http://delcorealestate.co.delaware.pa.us/pt/search/commonsearch.aspx?mode=parid` | Redirects to a disclaimer the user must agree to ("By using this information, the User is stating that the above Disclaimer has been read…"); page contains reCAPTCHA elements. The county removed name search and photos for privacy. | **User-assisted** (`delco.assessment_portal`). Open the county link, agree yourself, then paste Site Information, Residential/Commercial, Owner History, Tax Sale Information and Delinquent Tax. Daily refresh creates re-capture tasks. |
