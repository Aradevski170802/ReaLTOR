...
| Prothonotary case search (Common Pleas civil / municipal liens) | `http://webapp.montcopa.org/PSI/Viewer/Search.aspx?c=CaseSearch&panel=CaseNumber` | Redirects to `courtsapp.montcopa.org/psi/auth/init` with a "Validating" human-verification page. | **User-assisted** (`montco.civil`). Search by parcel with dashes removed. Only Status "1 - Open" cases whose type includes "Lien" are kept. The human-verification gate is never bypassed, so there is no automated route to the county's municipal-lien index. |
| PA UJS Portal — Case Search | `https://ujsportal.pacourts.us/CaseSearch` | 2026-09-15: no login, **no CAPTCHA**, no robots.txt restriction; Terms page is a liability disclaimer only. Participant-name search (County=Montgomery, Docket Type=Civil) returns **Magisterial District** civil dockets (money judgments, landlord/tenant) — *not* Common Pleas municipal liens. | **Automated browser when enabled** (`montco.ujs_judgments`). Runs a Participant Name search per derived owner name and keeps only cases with the owner on the **defendant** side (a possible judgment lien / distress signal), each flagged for review. A judgment is not a recorded municipal lien. Falls back to user-assisted capture when browser automation is off. |
